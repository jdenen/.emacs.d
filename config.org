...
I use a lot of Rake tasks, so a task runner is nice. I use =rake.el=.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
*** JSON
I work with a lot of JSON responses, so parsing them into a readable format is 
a must. I prefer 2 space indentation.
