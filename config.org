...
I monitor my local system with =symon=, but I only want to watch CPU and memory
usage.
...
#+BEGIN_SRC emacs-lisp
  (setq org-capture-templates
        '(("n" "Note" entry (file+datetree "~/vc-docs/scratch.org")  ;; No action, no deadline
...
And since I don't want to close my pinned window with =C-x 1=, I bind it to 
=pinup-delete-other-windows=.
...
#+BEGIN_SRC emacs-lisp
  (use-package pinup
    ...
    :bind
    ("C-x 1" . pinup-delete-other-windows))
#+END_SRC
...
I use =yankpad= to define my snippets with =org=.

#+BEGIN_SRC emacs-lisp
  (use-package yankpad
    :ensure t
    :init 
    (setq yankpad-file "~/vc-docs/yankpad.org")
    :bind
    ("C-c y" . yankpad-map))
#+END_SRC

=yankpad= is dependent on =yasnippet= for expansion, so install that too.
...
I use =gnus= to manage RSS feeds and watch mailing lists.
...
#+BEGIN_SRC emacs-lisp
  (defun my/remote-shell (user host)
    ...
    (let ((default-directory (concat "/ssh:" user "@" host ":/home/" user)))
      (shell (concat "*" host "*"))))
  ...
  (key-chord-define-global "jt" 'eshell)
#+END_SRC
...
I've recently discovered this package thanks to Emacs GIFs (@emacs_gifs) on twitter.
...
#+BEGIN_SRC emacs-lisp
  (use-package helm
    ...
    (setq helm-quick-update                     t
          ...
          helm-locate-command                   "mdfind -name %s %s") ; Mac only
...
=helm-swoop-without-pre-input= because I often swoop after opening a file.
...
I can capture regions (=expand-region=) and sexps (=mark-sexp=) for =hydra= use.
...
#+BEGIN_SRC emacs-lisp
  (defun my/expand-region ()
    ...
    (interactive)
    (my/expand-into-hydra 'er/expand-region 1))

  (defun my/expand-sexp ()
    "Expand by sexp into hydra."
    (interactive)
    (my/expand-into-hydra 'mark-sexp))

  (defun my/expand-into-hydra (expander &optional args)
    "Expand region by EXPANDER function and open `hydra/expand'."
    (progn
...
#+BEGIN_SRC emacs-lisp
  ...
  (key-chord-define-global "::" 'my/expand-sexp)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package twittering-mode
    ...
    (key-chord-define twittering-mode-map "nr" 'twittering-native-retweet)
    :bind
...
#+BEGIN_SRC emacs-lisp
  (use-package vimish-fold
    :ensure t
    :bind
    ("C-c v" . vimish-fold-delete)
    ("C-c V" . vimish-fold-delete-all))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (bind-key "S-<return>" 'new-line-below)
  (bind-key "S-<return>" 'new-line-below org-mode-map)
#+END_SRC
