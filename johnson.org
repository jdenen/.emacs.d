...
#+BEGIN_SRC bash
cd ~ && git clone git@github.com:jdenen/.emacs.d
#+END_SRC
*** Org config
...
`diminish` hides a bunch of mode line information that I do not care to know. These are typically modes that I'm *always* in.
#+BEGIN_SRC emacs-lisp
(johnson/package-install `diminish)
